        ...
        if ($request->isMethod('get')) {
            ...
            if (!empty($request)) {
                ...
                if (!empty($users['data'])) {
                    ...
                    // dd($num);
                    // 从数据库获得对应数据
                    $userss = User::where('name', $num)->orWhere('name','like','%'.$num.'%')
                        ->first();
                    // dd($userss);
                    ...
                    if (!empty($userss)) {
                        // dd($userss);
                        // 分页
                        $userss = User::where('name',$num)->orWhere('name','like','%'.$num.'%')->paginate(10);
                        return view('users/index', [
                            'userss' => $userss,
                        ]);
                    } else {
    ...
    public function selectID(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            ...
            if (!empty($users['data'])) {
                ...
                $userss0 = User::where('id', $num)
                    ->get();
                // 用count（）判断object类型是否为空 否则查无此人
                if (count($userss0)) {
                    // dd($userss);
                    // 分页
                    $userss = User::where('id', $num)->paginate(10);
                    return view('users/index', [
                        'userss' => $userss,
                    ]);
                } else {
